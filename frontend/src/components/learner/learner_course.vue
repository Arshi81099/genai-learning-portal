      ...
      <main class="main-content">
        ...
        <div v-else-if="selectedContent === 'video'">
          ...
          <div v-else>
            ...
            <button class="summary-btn" @click="getVideoSummary">Summary by AI</button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    try {
      const videoId = await this.extractVideoId(this.video.link);
      console.log(videoId); // Use the variable directly
      if (videoId) {
        const response = await axios.get('/api/video/summary', { params: { videoId } });
        alert(response.data.message); // Display the summary
      } else {
        alert('Video ID not found.');
      }
    } catch (error) {
      console.error('Error getting video summary:', error);
      alert('Error fetching video summary.');
    }
  },
...
.submit-btn, .ai-help-btn, .summary-btn {
...
.submit-btn:hover, .ai-help-btn:hover, .summary-btn:hover {
